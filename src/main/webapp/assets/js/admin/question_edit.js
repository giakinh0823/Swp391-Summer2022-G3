...
explain.attachEvent("exec_command_delete", function () {
    if($(explain.getSelectionElement()).attr("src")){
        if (`${$(explain.getSelectionElement()).attr("src")}`.indexOf("data")<0) {
        ...
        $.ajax({
            ...
            error: function(error){
                ...
                if(error.responseJSON.message){
                    ...
                    $("#message-alert-answer-error").text("Some thing error!");
                }
            }
        })
    }, 500);
    ...
    $.ajax({
        ...
        error: function (error) {
            ...
            if(error.responseJSON?.message){
                 ...
                 $("#message-create-answer-error").text("Some thing error!");
            }
        }
    });
})
...
$("#form-question").on("submit", function (event) {
    if (listImages!=null && listImages.length > 0) {
        $.ajax({
            method: "POST",
            url: contextPath + "/admin/question/media/delete",
            ...
            error: function(error){
                ...
                if(error.responseJSON?.message){
                    ...
                    $("#message-create-question-error").text("Some thing error!");
                }
            }
        })
    }
    ...
    $("#create-question-success").addClass("hidden");
    ...
    setTimeout(function(){
        $.ajax({
            method: 'POST',
            url: contextPath + '/admin/question/edit/'+questionId,
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
            dataType: 'json',
            success: function (data) {
                $("#create-question-success").removeClass("hidden");
                $("#message-create-question-success").text("Update success!");
                $("#button-question-model-spinner").click();
            },
            error: function (error) {
                $("#create-question-error").removeClass("hidden");
                if(error.responseJSON?.message){
                     ...
                     $("#message-create-question-error").text("Can't save! some thing error!");
                }
                $("#button-question-model-spinner").click();
            }
        });
