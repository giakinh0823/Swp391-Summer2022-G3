...
const deleteDimension = (id) => {
    ...
    for (var i = 0; i < listDimensions.length; i++) {
        html += `<div class="flex mb-3">
                        ...
                        <button type="button" class="ml-2" onclick="saveDimension(${listDimensions[i].question}, ${listDimensions[i].id}, '${listDimensions[i].name}')">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>                                
                        </button>
                   </div>`
    }
...
$("#form-quizzes").on("submit", function(event){
    ...
    $.ajax({
        method: "POST",
        url: contextPath + "/admin/quizzes/create",
        data: from.serialize(),
        dataType: 'json',
        success: function(data){
            console.log(data);
