...
const label_customers = [];
const customers = [];
const label_registers = [];
const success_registers = [];
const pending_registers = [];
const label_subjects = [];
const create_subjects = [];
const update_subjects = [];
for (const [key, value] of data_customers) {
  label_customers.push(key);
}
for (const [key, value] of data_registers_success) {
  label_registers.push(key);
}
for (const [key, value] of data_subjects_create) {
  label_subjects.push(key);
}
label_subjects.sort((a, b) => new Date(a).getTime() - new Date(b).getTime());
label_subjects.forEach(key => {
    create_subjects.push(data_subjects_create.get(key));
    update_subjects.push(data_subjects_update.get(key));
});
label_registers.sort((a, b) => new Date(a).getTime() - new Date(b).getTime());

label_registers.forEach(key => {
    success_registers.push(data_registers_success.get(key));
    pending_registers.push(data_registers_pending.get(key));
});

label_customers.sort((a, b) => new Date(a).getTime() - new Date(b).getTime());
label_customers.forEach(key => {
   customers.push(data_customers.get(key));
});
...
var options_subject = {
    series: [{
            name: 'CREATE',
            type: 'column',
            data: [...create_subjects]
        }, {
            name: 'UPDATE',
            type: 'area',
            data: [...update_subjects]
        }],
    chart: {
        height: 650,
        type: 'line',
...
var chart_subject = new ApexCharts(document.querySelector("#chart-subject"), options_subject);
...
var spark1 = {
  ...
  colors: ['#26dec1'],
  title: {
    text: create_subjects.reduce((total, item) => total + item, 0),
    offsetX: 30,
    style: {
      fontSize: '24px',
...
var spark2 = {
  ...
  colors: ['#f78da7'],
  title: {
    text: update_subjects.reduce((total, item) => total + item, 0),
    offsetX: 30,
    style: {
      fontSize: '24px',
      cssClass: 'apexcharts-yaxis-title'
...
var spark3 = {
  ...
  series: [{
    name: 'Registers',
    data: [...success_registers],
  }],
  labels: [...label_registers],
  xaxis: {
    type: 'datetime',
  },
  yaxis: {
  ...
  //colors: ['#5564BE'],
  title: {
    text: success_registers.reduce((total, item) => total + item, 0),
    offsetX: 30,
    style: {
      fontSize: '24px',
      cssClass: 'apexcharts-yaxis-title'
