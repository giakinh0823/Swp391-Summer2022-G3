<%-- 
    ...
    Created on : Jun 8, 2022, 10:09:50 PM
    Author     : giaki
--%>
...
<div class="container mx-auto p-5">
    <form class="mt-5" method="POST" enctype="multipart/form-data" id="form-question">
        <div class="p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg hidden" role="alert" id="create-question-error">
        ...
        <div class="grid grid-cols-1 2xl:grid-cols-2 space-x-10 mt-10">
            <div>
                <div class="max-w-xl mx-auto 2xl:max-w-full">
                    <div class="mb-6">
                        <div class="mb-6">
                            <label for="status" class="inline-flex relative items-center cursor-pointer">
                                <input type="checkbox" name="status" ${status=='active' ? "checked" : ""} value="true" id="status"  class="sr-only peer">
                        ...
                        <div class="min-h-[180px] max-h-[200px] overflow-y-auto px-2 pt-3" id="list-subject">
                            <c:forEach items="${subjects}" var="item">
                                <div class="flex items-center mb-2">
                                    <input type="radio" value="${item.id}" onchange="selectSubject(this)" name="subject" id="subject-${item.id}"  class="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 focus:ring-2"/>
                    ...
                    <div class="mb-6 hidden" id="lesson-content">
                        ...
                        <div class="min-h-[180px] max-h-[200px] overflow-y-auto px-2 pt-2" id="list-lesson">
                            <div class="flex items-center mb-2">
                    ...
                    <div class="hidden" id="not-found-dimension">
                        <div class="p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg" role="alert">
                            <span class="font-medium">Danger alert!</span> You need add dimension for current subject selected.
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <select id="level" name="level"
                            ...
                            <c:forEach items="${levels}" var="item">
                                <option value="${item.id}" ${level==item.id ? "selected": ""}>${item.value}</option>
                            </c:forEach>
                        </select>
                    </div>
                    <div class="mb-6">
                        <div class="mb-6">
                            <div class="flex justify-center items-center w-full">
                                <label for="media" class="relative flex flex-col justify-center items-center w-full h-64 bg-gray-50 rounded-lg border-2 border-gray-300 border-dashed cursor-pointer hover:bg-gray-100">
                                    ...
                                    <div class="flex flex-col justify-center items-center pt-5 pb-6">
                                        ...
                                        <div id="icon-upload" class="flex items-center flex-col justify-center">
                                            ...
                                            <p class="mb-2 text-sm text-gray-500 "><span class="font-semibold">Click to upload media!</span>
                                        </div>
                                    </div>
                                    <input id="media" name="media" type="file" class="hidden" accept="image/*, audio/*, video/*"/>
                                </label>
                            </div> 
                        </div>
                    </div>
            ...
            <div>
                ...
                <div class="mb-6 hidden" id="search-answer-content">
                    <label for="search-answer" class="block mb-2 text-sm font-medium text-gray-900">Answer</label>
                    <div class="w-full mb-4">
                        <div class="relative w-full">
                            <div class="flex absolute inset-y-0 left-0 items-center pl-3 pointer-events-none">
                                <svg class="w-5 h-5 text-gray-500" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clip-rule="evenodd"></path></svg>                            
                            </div>
                            <input type="text" onclick="$('#button-search-answer-modal').click(); openSearchAnswer();" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2 " placeholder="Search answer in current subject">
                        </div>
                    </div>
                    <div class="mb-5">
                    ...
                    <div class="flex items-center justify-center mb-3">
                        <button type="button" id="button-open-create-answer"  data-modal-toggle="new-answer-modal" class="flex items-center justify-center py-2 px-3 mr-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200">
                ...
                <div class="mb-6">
                    <label for="explain" class="block mb-2 text-sm font-medium text-gray-900">Explain</label>
...
<button id="button-search-answer-modal" class="hidden w-full md:w-auto text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center" type="button" data-modal-toggle="search-answer-modal"></button>
<div id="search-answer-modal" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-[1000] w-full md:inset-0 h-modal md:h-full">
    <div class="relative w-full h-full  max-w-2xl">
        <div class="bg-white rounded-lg p-4 mt-5">
            <div class="flex justify-end mb-5">
                <button class="hover:bg-gray-100 rounded-full" type="button" data-modal-toggle="search-answer-modal">
                ...
                <div class="space-y-6">
                    <div class="w-full mb-4">
                        <div class="relative w-full">
                            ...
                            <input type="text" id="search-answer" oninput="searchItemFilterAnswer(this)" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2 " placeholder="Search answer in current answer">
                        </div>
                    </div>
                    <div class="p-4 mb-2 text-sm text-green-700 bg-green-100 rounded-lg hidden" role="alert" id="alert-answer-success">
                        <span class="font-medium">Success!</span> <span id="message-alert-answer-success"></span>
                    </div>
                    <div class="p-4 mb-2 text-sm text-red-700 bg-red-100 rounded-lg  hidden" role="alert" id="alert-answer-error">
                        <span class="font-medium">Error!</span> <span id="message-alert-answer-error"></span>
                    </div>
                    <div class="min-h-[180px] max-h-[500px] overflow-y-auto px-2 pt-3" id="table-answer-search">
                        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                            <table class="w-full text-sm text-left text-gray-500">
                        ...
                        <div id="spin-search-answer" class="flex justify-center items-center hidden mt-10">
                            <svg role="status" class="w-8 h-8 mr-2 text-gray-200 animate-spin fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"/>
                                <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/>
                            </svg>
...
<div id="new-answer-modal" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-[1000] w-full md:inset-0 h-modal md:h-full">
    <div class="relative w-full h-full  max-w-2xl">
        <div class="bg-white rounded-lg p-4 mt-5">
            <div class="flex justify-end mb-5">
                <button class="hover:bg-gray-100 rounded-full" type="button" data-modal-toggle="new-answer-modal">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                </button>
            </div>
            <!-- Modal content -->
            <div class="relative bg-white">
                <div class="space-y-6">
                    <form class="pb-5 px-5" action="${pageContext.request.contextPath}/admin/question/answer/create" method="POST" id="form-create-answer">
                        <div class="p-4 mb-2 text-sm text-green-700 bg-green-100 rounded-lg hidden" role="alert" id="create-answer-success">
                            <span class="font-medium">Success!</span> <span id="message-create-answer-success"></span>
                        </div>
                        <div class="p-4 mb-2 text-sm text-red-700 bg-red-100 rounded-lg  hidden" role="alert" id="create-answer-error">
                            <span class="font-medium">Error!</span> <span id="message-create-answer-error"></span>
                        </div>
                        <input type="hidden" name="subject" id="create-answer-subject"/>
                        <div class="mb-6">
                            <label for="text" class="block mb-2 text-sm font-medium text-gray-900">Text</label>
                            <textarea name="text" id="text" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Text question..."></textarea>
                        </div>
                        <div class="mb-6">
                            <label for="media-answer" class="relative flex flex-col justify-center items-center w-full h-64 bg-gray-50 rounded-lg border-2 border-gray-300 border-dashed cursor-pointer hover:bg-gray-100">
                                ...
                                <div class="flex flex-col justify-center items-center pt-5 pb-6">
                                    ...
                                    <div id="icon-upload-answer" class="flex items-center flex-col justify-center">
                                        <svg class="mb-3 w-10 h-10 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path></svg>
                                        <p class="mb-2 text-sm text-gray-500 "><span class="font-semibold">Click to upload</span> or drag and drop</p>
                                    </div>
                                </div>
                                <input id="media-answer" name="media" type="file" accept="image/*" class="hidden" />
                            </label>
                        </div>

                        <div class="flex justify-end">
                            <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2 text-center">Add</button>
                        </div>
                    </form>
                </div>
            </div>
...
<div id="popup-modal-delete" tabindex="-1" class="hidden z-[10000] overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 md:inset-0 h-modal md:h-full">
    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
        <div class="relative bg-white rounded-lg shadow-lg">
            <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center " id="button-model-answer-delete" data-modal-toggle="popup-modal-delete">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>  
            </button>
            <div class="p-6 text-center">
                ...
                <h3 class="mb-5 text-lg font-normal text-gray-500">Are you sure you want to delete this answer?</h3>
...
<button type="button" id="button-question-model-spinner" data-modal-toggle="create-question-model-spinner"></button>
<div id="create-question-model-spinner" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full">
