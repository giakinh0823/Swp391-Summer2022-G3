...
<div class="container mx-auto mt-20">
    <form class="max-w-[450px] mx-auto" method="POST">
        ...
        <div class="mb-6">
            ...
            <input type="text" id="name" name="name" value="${dimension.name}" ${sessionScope.user.checkFeature('DIMENSION_EDIT') || sessionScope.user.is_super() ? "": "disabled"} class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
        </div>
        <div class="mb-6">
            <label for="type" class="block mb-2 text-sm font-medium text-gray-900">Type</label>
            <select id="type" name="type" ${sessionScope.user.checkFeature('DIMENSION_EDIT') || sessionScope.user.is_super() ? "": "disabled"} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
        ...
        <div class="mb-6">
            ...
            <textarea id="description" name="description" rows="6" ${sessionScope.user.checkFeature('DIMENSION_EDIT') || sessionScope.user.is_super() ? "": "disabled"} class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Your description...">${dimension.description}</textarea>
        </div>
        <c:if test="${sessionScope.user.checkFeature('DIMENSION_EDIT') || sessionScope.user.is_super()}">
